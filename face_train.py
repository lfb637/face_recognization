...
class Dataset:
    ...
    def load(self, img_rows=IMAGE_SIZE, img_cols=IMAGE_SIZE,img_channels=3):
        ...
        _, test_images, _, test_labels = train_test_split(images, labels, test_size=0.2, random_state=random.randint(0, 100))  # 测试集样本

        # 当前的维度顺序如果为'th'，则输入图片数据时的顺序为：channels,rows,cols，否则:rows,cols,anchnels
        ...
        if K.image_dim_ordering() == 'th':
            ...
            # 使用categorical_crossentropy作为损失函数（交叉熵函数），需要根据类别数量nb_classes将
            # 类别标签进行one-hot编码使其向量化
            # one-hot编码：解决了分类器不好处理属性数据的问题；在一定程度上起扩充特征的作用
...
class Model:
    ...
    def build_model(self, dataset, nb_classes):
        # 构建一个空的网络模型，它是一个线性堆叠模型，各神经网络层顺序添加
        self.model = Sequential()

        # 将顺序添加CNN网络需要的各层，一个add就是一个网络层
        self.model.add(Convolution2D(32, (3, 3), padding='same',
                                     input_shape=dataset.input_shape))  # 1 2维卷积层卷积核个数，大小，步长（1）
        ...
        self.model.add(Dense(512))  # 14 Dense层(全连接层)
    ...
    def train(self, dataset, batch_size=20, epochs=10, data_augmentation=True):
        sgd = SGD(lr=0.0007, decay=1e-6, momentum=0.9, nesterov=True)  # 采用SGD+momentum的优化器进行训练，首先生成一个优化器对象
    ...
    # MODEL_PATH = 'F:/bs_data/others_2.h5'
    ...
    def evaluate(self, dataset):
        ...
        print("%s: %f   %s: %.2f%%" % (self.model.metrics_names[0], score[0],
                                       self.model.metrics_names[1], score[1] * 100))        # 测试集loss，acc
...
if __name__ == '__main__':
    dataset = Dataset('F:/bs_data/')
    dataset.load()
    model = Model()
    model.build_model(dataset, dataset.nb_classes)
    print("= = = = = = = = 模型训练= = = = = = = = = = =")
    model.train(dataset)
    model.save_model(file_path='F:/bs_data/lfb.h5')
    model.save_model(file_path='F:/bs_data/others_1.h5')
    model.save_model(file_path='F:/bs_data/others_2.h5')
    print("= = = = = = = = 测试集= = = = = = = = = = =")
